<?php 
   ...
   if(@$_SESSION['name']){
		   ...
		   if(!empty($_POST['generate'])){
			   ...
			   $_SESSION['year'] = $year;
			   $_SESSION['faculty'] = $facultyname;
			   
			   // check to see if time table already exists..
			   $query = $link->query("SELECT * FROM timetable WHERE semester='$semester' AND course='$course_code' AND course_full_name= '$course_full_name' AND faculty = '$facultyname' AND user_id='$user_id'");
			   $query->setFetchMode(PDO::FETCH_ASSOC);
			   ...
			   while($result = $query->fetch()){
					...
					if($id){
							   ...
							   $query = $link->prepare("INSERT INTO timetable (user_id,course_full_name,year,semester,course,faculty) VALUES(?,?,?,?,?,?)");
							   $values = array ($user_id,$course_full_name,$year,$semester,$course_code,$facultyname);
...
<div class="container"> 
  <div class="row">
    <div class="col-lg-6"><div class="jumbotron">
			You first need to Add Subjects, School, Class. 				
			...
			<div class="form-group">
			  ...
			  <div class="col-md-8">
				<select id="facultyname" name="facultyname" class="form-control" required="">
				<?php
				// lists the course on drop down button course
				$query = $link->query("SELECT DISTINCT faculty_name FROM faculty WHERE user_id='$user_id'"); 
				$query->setFetchMode(PDO::FETCH_ASSOC);	
				while($result = $query->fetch()){
			...
			<!-- Select Basic -->
			<div class="form-group">
			  <label class="col-md-4 control-label" for="semester">Select Term</label>
			  <div class="col-md-8">
			...
			<!-- Select Basic -->
			<div class="form-group">
			  <label class="col-md-4 control-label" for="Course">Class Code</label>
			  <div class="col-md-8">
				...
				<span class="help-block">class, term, section</span>  
			  </div>
			</div>
